...
gives a much more specifc message than the engine's generic parse
error would.
"""
...
def preflight(expr: str) -> ValidationResult:
    ...
    the extension point fo rmore (e.g. trailing operators, empty 
    function calls) without changing callers."""
    if not expr or not expr.strip():
        return ValidationResult(False, "the expresison is empty", 1, "")
