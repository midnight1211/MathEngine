...
# Feature: semantic-router confirmation ("did you mean X? (say yes and
# I'll run it)"). Deliberately short/anchored (fullmatch, not search) so
# a longer message that merely contains "yes" somewhere isn't mistaken
# for a confirmation - see nlp_engine.py's pending_suggestion handling.
AFFIRMATIVE_RE = re.compile(
    r"^(?:yes|yeah|yep|yup|sure|ok(?:ay)?|do it|run it|go ahead|"
    r"that(?:'s| is) (?:right|it|correct)|correct|please)[.!]?$",
    re.IGNORECASE,
)
...
def detect_precision_toggle(text: str) -> Optional[str]:
...
def is_affirmative(text: str) -> bool:
    """True for a short "yes"-shaped reply - used only to confirm a
    semantic_router suggestion (see nlp_engine.py's pending_suggestion),
    never as a general intent, so it deliberately stays narrow rather
    than trying to catch every way of saying yes."""
